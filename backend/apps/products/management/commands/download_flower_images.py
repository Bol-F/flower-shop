...
FLOWER_MAP = {
    'crimson-velvet-rose': ('Red_rose.jpg', 'red rose flower closeup macro'),
    'champagne-garden-rose': ('Rose_Amber_Flush_20070601.jpg', 'rose cultivar cream'),
    'midnight-blue-rose': ('Blue_Rose_(3931263892).jpg', 'purple blue rose flower'),
    'pink-whisper-spray-rose': ('Pink_roses_-_Flickr_-_maticsteve.jpg', 'pink spray rose cluster'),
    'eternal-white-rose': ('White_rose.jpg', 'white rose single closeup'),
    'parrot-tulip-fiesta': ('Tulipa_Estella_Rijnveld_2.jpg', 'parrot tulip'),
    'double-dutch-peach-tulip': ('Tulipa_-_double_pink_(aka).jpg', 'peach double tulip bloom'),
    'queen-of-night-tulip': ('Tulipa_-_Queen_of_Night.jpg', 'Queen of Night dark tulip'),
    'rainbow-spring-mix': ('Keukenhof_tulips.jpg', 'Keukenhof tulips'),
    'giant-helianthus': ('Sunflower_from_Silesia2.jpg', 'sunflower closeup large bloom'),
    'teddy-bear-sunflower': ('Helianthus_annuus_3.jpg', 'sunflower fluffy double bloom'),
    'autumn-rust-sunflower': ('Sunflower_Helianthus_annuus_Prado_Red.jpg', 'sunflower red'),
    'bird-of-paradise': (
        'Bird_of_Paradise_(Strelitzia_Reginae).jpg',
        'strelitzia bird of paradise orange tropical',
    ),
    'purple-vanda-orchid': ('Vanda_coerulea.jpg', 'Vanda coerulea'),
    'king-protea': ('Protea_cynaroides_2.jpg', 'Protea cynaroides'),
    'red-anthurium': ('Anthurium_andraeanum.jpg', 'Anthurium andraeanum'),
    ...
    'ranunculus-sunrise': ('Ranunculus_asiaticus_1.jpg', 'Ranunculus asiaticus'),
    'romance-in-red': ('Bouquet_de_roses_roses.jpg', 'red roses bouquet'),
    'garden-sunshine-bouquet': ('Sunflowers_in_a_vase.jpg', 'sunflower bouquet bright yellow vase'),
    'pastel-paradise': ('Pink_flower_bouquet.jpg', 'pink flower bouquet'),
    'eternal-love-premium': ('Flower_arrangement_-_geograph.org.uk.jpg', 'flower arrangement'),
    ...
    'meadow-magic-mix': ('Wildflower_meadow.jpg', 'wildflower meadow mixed colourful'),
}
...
def save_image(url, path, retries=4):
    for attempt in range(retries):
        try:
            r = requests.get(url, timeout=45, stream=True, headers={'User-Agent': USER_AGENT})
    ...
    def handle(self, *args, **options):
        ...
        for slug, (preferred_file, fallback_query) in FLOWER_MAP.items():
            ...
            try:
                ...
                if not thumb_url:
                    self.stdout.write(self.style.WARNING(f'  {product.name:<38} no image found'))
                    failed += 1
                    continue
                ...
                self.stdout.write(self.style.ERROR(f'  {product.name:<38} ERROR: {exc}'))
                failed += 1

            time.sleep(3)  # be polite to the API — avoid 429 rate limits

        self.stdout.write('\n' + self.style.SUCCESS(f'Done — {ok} downloaded,  {failed} failed.'))
