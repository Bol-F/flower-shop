  ...
  if (provider === "google") {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true" className="size-5">
        ...
        <path fill="#FBBC05" d="M6.4 13.93A6 6 0 0 1 6.09 12c0-.67.12-1.32.31-1.93V7.45H3.05A10 10 0 0 0 2 12c0 1.63.39 3.17 1.05 4.55l3.35-2.62Z" />
        <path fill="#EA4335" d="M12 5.94c1.47 0 2.79.5 3.83 1.5l2.87-2.88A9.62 9.62 0 0 0 12 2a10 10 0 0 0-8.95 5.45l3.35 2.62c.79-2.37 3-4.13 5.6-4.13Z" />
      </svg>
    );
  }
  if (provider === "github") {
...
export default function SocialLoginButtons({
  ...
  useEffect(() => {
    ...
    void fetchOAuthProviders()
      .then((items) => {
        if (active) setEnabled(new Set(items.filter((item) => item.enabled).map((item) => item.id)));
      })
      .catch(() => {
