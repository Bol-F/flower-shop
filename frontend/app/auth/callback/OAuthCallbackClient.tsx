...
export default function OAuthCallbackClient({
  ...
  useEffect(() => {
    ...
    void completeOAuth(code)
      ...
      .catch((reason: unknown) => {
        setFailure(reason instanceof Error ? reason.message : "This sign-in link is invalid or expired.");
      });
  ...
  if (failure || (!code && !error)) {
    return (
      <section className="mx-auto max-w-lg rounded-[2rem] border border-line bg-white p-7 text-center shadow-lift">
        <div className="mx-auto grid size-14 place-items-center rounded-2xl bg-berrysoft text-2xl">×</div>
        ...
        <Link href="/profile?mode=login" className="mt-6 inline-flex rounded-full bg-blossomdeep px-6 py-3 text-sm font-extrabold text-white shadow-glow">
          Return to sign in
        </Link>
      </section>
    );
  ...
  return (
    <section className="mx-auto max-w-lg rounded-[2rem] border border-line bg-white p-7 text-center shadow-lift" aria-live="polite">
      ...
      <p className="mt-2 text-sm font-semibold text-stone">Securely exchanging your one-time code…</p>
    </section>
  );
}
